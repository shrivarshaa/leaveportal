...
const FacultyDashboard = () => {
    ...
    const [classData, setClassData] = useState({ students: [], attendance: [] });
    const [selectedDate, setSelectedDate] = useState(new Date().toISOString().split('T')[0]);
    const [view, setView] = useState('leaves'); // 'leaves', 'attendance', or 'inbox'
    ...
    const fetchAttendance = async () => {
        ...
        const config = { headers: { Authorization: `Bearer ${token}` } };
        try {
            const { data } = await axios.get(`/api/attendance/class?date=${selectedDate}`, config);
            setClassData(data);
        } catch (error) {
            console.error(error);
    ...
    const markStudentAttendance = async (studentId, status) => {
        ...
        const config = { headers: { Authorization: `Bearer ${token}` } };
        try {
            await axios.post('/api/attendance/mark', { studentId, date: selectedDate, status }, config);
    ...
    return (
        <div className="max-w-7xl mx-auto mt-24 px-4 sm:px-6 lg:px-8 pb-12">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
                <div>
                    <h1 className="heading-primary text-3xl">Faculty Dashboard</h1>
                    <p className="text-gray-400 mt-1">Review leaves and manage class attendance</p>
                </div>
                <div className="flex bg-white/5 p-1 rounded-xl backdrop-blur-md border border-white/10">
                    <button
                        onClick={() => setView('leaves')}
                        className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${view === 'leaves' ? 'bg-primary text-white shadow-lg' : 'text-gray-400 hover:text-white'}`}
                    >
                        Leaves
                    </button>
                    <button
                        onClick={() => setView('attendance')}
                        className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${view === 'attendance' ? 'bg-primary text-white shadow-lg' : 'text-gray-400 hover:text-white'}`}
                    >
                        Attendance
                    </button>
                    <button
                        onClick={() => setView('inbox')}
                        className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${view === 'inbox' ? 'bg-primary text-white shadow-lg' : 'text-gray-400 hover:text-white'}`}
                    >
                        Messages {messages.filter(m => !m.isRead && m.recipient?._id === user?._id).length > 0 && <span className="ml-1 bg-red-500 text-[10px] px-1.5 rounded-full pulse">!</span>}
                    </button>
                </div>
                ...
                <div className="glass-card overflow-hidden">
                                    ...
                                    .map((leave) => (
                                        <tr key={leave._id} className={`hover:bg-white/5 transition-colors duration-200 ${leave.leaveType === 'Emergency' ? 'bg-red-500/5' : ''}`}>
                                            ...
                                            <td className="py-4 px-6 text-center">
                                                <span className={`px-3 py-1 rounded-full text-xs font-medium backdrop-blur-md border ${leave.facultyApproval.status === 'Approved' ? 'bg-green-500/20 text-green-300 border-green-500/30' :
                                            ...
                                            <td className="py-4 px-6 text-center">
                                                {leave.facultyApproval.status === 'Pending' && (
                    ...
                    {leaves.length === 0 && (
                        <div className="text-center py-12">
                            <p className="text-gray-400 italic">No leave applications to review.</p>
                        </div>
                    )}
                </div>
            )}
            ...
            {view === 'attendance' && (
                <div className="glass-card p-6">
                    <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-6 pb-6 border-b border-white/10">
                        <h2 className="text-xl font-bold text-white">Class Attendance Management</h2>
                        <div className="flex items-center gap-4 bg-white/5 p-2 rounded-xl border border-white/10">
                            <span className="text-xs text-gray-400 font-bold uppercase tracking-wider">Select Date:</span>
                            <input
                                type="date"
                                value={selectedDate}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {classData.students?.map((student) => {
                            const record = classData.attendance?.find(a => a.student === student._id);
                            return (
                                <div key={student._id} className="glass-card p-4 hover:border-primary/50 transition-all group">
                                    <div className="flex justify-between items-start mb-4">
                                        <div>
                                            <h4 className="font-bold text-white group-hover:text-primary transition-colors">{student.name}</h4>
                                            <p className="text-[10px] text-gray-500 font-mono">{student.studentId}</p>
                                        </div>
                                        <div className={`px-2 py-1 rounded text-[10px] font-bold uppercase ${record?.status === 'Present' ? 'bg-green-500/10 text-green-400' : record?.status === 'Absent' ? 'bg-red-500/10 text-red-400' : 'bg-gray-500/10 text-gray-400'}`}>
                                    ...
                                    <div className="flex gap-2">
                                        <button
                                            ...
                                            className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${record?.status === 'Present' ? 'bg-green-500 text-white shadow-lg shadow-green-500/20' : 'bg-white/5 text-gray-400 hover:bg-white/10'}`}
                                        >
                                            Present
                                        </button>
                                        <button
                                            onClick={() => markStudentAttendance(student._id, 'Absent')}
                                            className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${record?.status === 'Absent' ? 'bg-red-500 text-white shadow-lg shadow-red-500/20' : 'bg-white/5 text-gray-400 hover:bg-white/10'}`}
                                        >
                                            Absent
                                        </button>
                                    </div>
            ...
            {view === 'inbox' && (
                <div className="glass-card h-[600px] flex flex-col overflow-hidden">
                    <div className="flex flex-1 overflow-hidden">
                        {/* Sidebar: Student List (who sent messages) */}
                            ...
                            {selectedStudent ? (
                                <>
                                    ...
                                    <form onSubmit={sendMessage} className="p-4 border-t border-white/10 bg-white/5 flex gap-2">
                                        ...
                                        />
                                        <button type="submit" className="p-3 bg-primary hover:bg-primary-hover text-white rounded-xl transition-all shadow-lg shadow-primary/20">
                                            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 19l7-7-7-7M5 12h14" /></svg>
                                        </button>
                                    </form>
                                </>
                            ) : (
